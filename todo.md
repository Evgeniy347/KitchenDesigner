...
- [ ] `KitchenGroup` (id/имя/список/Movable). Привязка/отвязка элементов.
- [ ] ПКМ-клик по объекту → мини-меню (новый `RmbMenuUI`): «Связать» (для
  выделенной группы) / «Настройки группы» (если объект уже в группе). ПКМ-drag
  → орбита (разделить клик/тянуть в `CameraController`, как ЛКМ в `ElementMover`).
- [ ] Связанные: ЛКМ по любому → выделяется вся группа; ПКМ → окно «Группа»
  (имя / замок закрыт / «Запретить перемещение»); перемещение целиком.
- [ ] Замок открыт → связь рвётся, окно группы закрывается.
- [ ] Сериализация групп в проект (id группы у элементов).
- [ ] Тесты: связать/разорвать, выбор всей группы, перемещение группы.

...
## 🆕 Сессия 2026-06-16 (раунд 3) — баг-фиксы захвата

- [x] **Прыжок полускрытой стены при захвате** (тест-фёрст): при захвате опущенной
  стены база перемещения бралась в опущенном состоянии, затем стена восстанавливалась
  на полную высоту → объект улетал (низ под пол). Фикс: `ElementMover.GrabStart`/
  `RestoreFull` возвращают стену на полную высоту ДО взятия старта; `BeginDrag`
  пересчитывает старт/offset. Регресс-тест `WallTests.LoweredWall_Grab_RestoresFull...`
  (сначала падал, после фикса зелёный).
- [x] **«Магический» ресайз вместо перемещения**: недетерминированный порядок
  `Update` — `ResizeHandleManager` иногда отрабатывал клик раньше `SelectionManager`
  и цеплял осиротевшую ручку прежнего выделения. Фикс: `[DefaultExecutionOrder(100)]`
  (ввод ручек — после выбора/перемещения) + грабельной сделана только выступающая
  часть стрелки (наконечник), клик по телу объекта теперь двигает, а не растягивает.

---

## 🆕 Сессия 2026-06-16 (раунд 2) — доработки ресайза, камера, режимы

- [x] **Запрет ресайза при locked** — у объекта с «Запретить перемещение» ручки
  не показываются (ресайз запрещён вместе с перемещением).
- [x] **Ресайз стены → пол** — `WallManager` не опускает ресайзимую стену
  (`IsResizingElement`), прилипание к полу снова работает.
- [x] **Прилипание при ресайзе в угол («Г»)** — убран жёсткий порог перекрытия
  0.3 в `ResizeSnap`; математика вынесена в `ResizeMath`; тесты `ResizeMathTests`
  (доски и стены: рост, снэп заподлицо, угловой стык, стена→пол, вне порога).
- [x] **Сохранение камеры** — положение/углы/дистанция камеры пишутся в проект
  (`ProjectData.camera`) и восстанавливаются при открытии сейва.
- [x] **ПКМ-клик → окно настроек** объекта (для группы/мультивыделения — меню группы).
- [x] **Переключатель режима ручек** в тулбаре: «Растяжение ↔ Перемещение по оси».
  В режиме перемещения наконечник — конус-стрелка, объект двигается вдоль одной
  оси со снэпом; в режиме растяжения — кубик (как было).

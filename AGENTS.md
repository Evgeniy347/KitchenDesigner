...
| `agents/GIT.md` | коммиты, `git-commit.ps1`, правила таймстемпов, push, временные файлы | перед любым коммитом |
...
| `docs/TODO.md` | технический долг: что не так, где, почему важно и что считается сделанным | когда есть время на долг, и перед тем как заводить новый пункт |

...
### Обойти сторожа — это нарушить правило и спрятать нарушение

Сторож — запись правила, а не регулярное выражение. За одну ночь его обошли дважды: полным
именем типа вместо `using`, чтобы скан не увидел ссылку между слоями, и рефлексией в приватный
метод движка, чтобы тест увидел то, чего продукту не нужно. Оба прогона стали зелёными, оба
правила остались нарушенными — и второе нарушение хуже первого, потому что его больше не
видно.

Если правило верно — удовлетвори его по существу: перенеси тип туда, откуда его законно
видят, поменяй направление зависимости, дождись кадра честно. Если правило неверно —
**расширь список разрешённого с причиной и доложи**, ровно как это делается для
`MayKnowTheSeatRule` и списков разрешённых файлов у соседних сторожей. Третьего пути нет.

Менеджеру для приёмки хватает одной проверки: зависимость появилась, а сторож молчит — ищи
полное имя.

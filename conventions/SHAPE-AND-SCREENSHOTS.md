...
**Пропорция кадра живёт в двух местах, и оба обязаны совпасть.** Камера обязана получать `aspect`
ровно той цели, в которую рисует, а цель — форму того прямоугольника, где картинку в итоге
покажут: `RawImage`, в отличие от `Image`, не умеет `preserveAspect` и растягивает текстуру молча.
Квадратная миниатюра 128×128 в плитке 96×56 сплющивалась в 0,58 — сфера светильника выглядела
яйцом на боку, — и рендер при этом был честным, так что сенсор на кадре промолчал бы. Сторожа два:
круглый объект оставляет круглый отпечаток **на НЕквадратной цели** (на квадратной дефект не виден
вовсе — тот же «тест на симметричном входе»), и потребитель заказывает текстуру формы своего
прямоугольника.

**Лицо элемента смотрит в +Z, а общая изометрическая камера стоит на −Z — то есть в затылок
каждому типу.** «Спинкой к зрителю» на миниатюре оказались стул и унитаз не потому, что они
особенные: так стоят все, просто у шкафа затылок похож на лицо. Разворот выводится из одной
объявленной фронтальной оси (`ElementFacing.LocalFront`), а не вписывается числом на тип; `IsoDir`
при этом не правится — он кадрирует все изометрические эталоны.

**Накладки сцены не фильтруются маской камеры.** `EdgeOutlineRenderer` рисует рёбра через `GL` в
`OnRenderObject`, а этот колбэк приходит на КАЖДУЮ камеру и `cullingMask` не смотрит: контур
настоящей сцены попадал в кадр миниатюры, хотя её элемент живёт в песочнице и в реестре его нет.
Такие рисовальщики гасят на время рендера явно и возвращают **как были** — выключенное чужой
рукой само не включается.

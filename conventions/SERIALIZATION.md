...
## Поле, которого эта версия не знает, переживает круг только СЫРЫМ текстом

`JsonUtility` молча выбрасывает незнакомый ключ, поэтому запись элемента обязана доживать от
чтения файла до записи нетронутой (`ElementData.rawJson` → `RawElementRecords`), а тип
объекта обязан ехать в файл СТРОКОЙ (`elementType`): по флагам вида `isXxx` старая версия не
отличит незнакомый тип от обычной детали — флага с таким именем у неё просто нет.

## Два вида, отличающиеся только названием, — это перечисление на ОДНОМ классе

Не второй класс. Наружу они выходят двумя именами типа в `CreateItem.type` и двумя строками
сайдбара, как `drawer`/`movento_drawer`, и одним `[Undoable]` свойством в панели. И два вида
обязаны получить РАЗНЫЕ `SidebarItemKind`: плитки каталога группируются по виду, одинаковый
склеивает их в одну кнопку с заголовком первой — именно так «сушильная машина» пропала из
интерфейса, существуя при этом в коде.

...
## Голый `Destroy(` — не стиль, а закрытый для EditMode путь

Вне play mode он БРОСАЕТ, и всё, что его зовёт, перестаёт быть проверяемым. Так `ElementMover`
падал, как только перетаскивание что-нибудь подкрасило, — и покадровую цену драга не стерёг
никто именно поэтому, а не по недосмотру. Даже в play mode отложенный снос живёт до конца
кадра, поэтому перестроение списка успевает удвоиться. Уничтожает в продакшене только
`DestroyNow.The`; сторож — `DestroyModeSingleDecisionTests`.

## Параметр правила отбирают у вызывающих

Если одно правило спрашивают с двух путей, параметр в сигнатуре — это приглашение каждому
пути выбрать себе своё значение, и они разъедутся. Так и разъехались радиусы шлюза: панель
судила нарушение на самой детали, жесты — в радиусе `snapThreshold × 2`, и один и тот же
запрет срабатывал на жесте и молчал в панели. Узкий радиус при этом был не строже, а слепее
ровно на один класс: он не видел, что правка увела опору из-под СОСЕДА. Параметр ушёл внутрь
`EditGate`, а единственность стережётся рефлексией по API плюс сканом на выражение.

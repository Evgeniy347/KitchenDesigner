...
export const GEN_TOOLS: GenTool[] = [
  ...
  },
  {
    ...
    kind: "write",
    inputSchema: {
      selector: z.string().min(1).describe("Selector over the scene. Space-separated clauses (AND): name mask \u0027B4_*\u0027, \u0027name:PAT\u0027, \u0027type:board|wall|floor|window|door|drawer|facade|assembled_facade|radial_shelf|panel|table|pillar|screw_leg|light\u0027, \u0027module:NAME\u0027, \u0027thickness==18\u0027 (also width/height/depth with == != \u003E= \u003C= \u003E \u003C), \u0027all_boards\u0027, \u0027all_modules\u0027, \u0027*\u0027."),
  ...
  },
  {
    ...
    inputSchema: {
      ...
      sill_mm: z.number().int().min(0).optional().describe("Height from wall base to opening bottom in MM. Must be 0 for a door: a door opening starts at the floor."),
    },
  },
  {
    name: "snap_diagnose",
  ...
  },
  {
    ...
    description: "Create one or MANY elements in ONE call. Each item: unique name, type (board | wall | floor | facade | assembled_facade | radial_shelf | panel | drawer | movento_drawer | table | radius_table | stool | chair | sofa | pouffe | bed | pillar | screw_leg | window | door | sink | cooktop | oven | dishwasher | toilet | wall_hung_toilet | bathtub | bath_mixer | shower_column | socket | light_switch, default board; an unknown type is REJECTED, the whole batch with it), x/y/z in METERS, width/height/depth in MILLIMETERS, plus type-specific fields (gaps, fill, drawer params, ...). sink and cooktop are RECESSED into a worktop: create them above one and they seat themselves. cooktop, oven and dishwasher are built-in appliances: they take their size from the manufacturer and ignore width/height/depth (a cooktop only when given model:...); a dishwasher has no front of its own \u2014 attach a facade to it with edit_elements attached_facade_name. panel = \u0414\u0412\u041F/\u0425\u0414\u0424 back panel whose gaps count toward its bounding box, so it seats into grooves. Atomic: if ANY item is invalid, NOTHING is created. Whole batch is ONE undo step. For architecture (walls, room floors, windows/doors) prefer the corner-anchored tools apply_floorplan / create_walls / create_floor / add_opening \u2014 they derive the geometry server-side instead of making you compute centers.",
    kind: "write",
    inputSchema: {
      items: z.array(z.object({ name: z.string().min(1).describe("Unique name for the new element (case-insensitive across the whole project). Must match ^[A-Za-z0-9_-]\u002B$ \u2014 latin letters, digits, \u0027-\u0027 and \u0027_\u0027 only; no spaces, no cyrillic."), type: z.enum(["board", "wall", "floor", "facade", "assembled_facade", "radial_shelf", "panel", "drawer", "movento_drawer", "table", "radius_table", "stool", "chair", "sofa", "pouffe", "bed", "pillar", "screw_leg", "window", "door", "sink", "cooktop", "oven", "dishwasher", "toilet", "wall_hung_toilet", "bathtub", "bath_mixer", "shower_column", "socket", "light_switch"]).optional().describe("Element type. Default board. wall = board acting as a structural anchor; floor ignores size/position. An unknown type is rejected and the whole batch with it."), x: z.number().finite().optional().describe("Position X in METERS."), y: z.number().finite().optional().describe("Position Y in METERS."), z: z.number().finite().optional().describe("Position Z in METERS."), width: z.number().int().min(1).optional().describe("Size along X in MM. Defaults: board 800, assembled facade 450, radial shelf 600, table 2000, window 900, door 900."), height: z.number().int().min(1).optional().describe("Size along Y in MM. Defaults: board 400, assembled facade 700, table 750, window 1200, door 2000."), depth: z.number().int().min(1).optional().describe("Thickness along Z in MM. Defaults: board 18, radial shelf 400 (its depth), table 1000, window/door 100."), model: z.enum(["Bosch PUE611BB5E", "Bosch HBA514BB3", "Bosch SMV25EX02E"]).optional().describe("Appliance model for a built-in appliance (type cooktop). Its size and cutout come from the manufacturer and cannot be edited afterwards; width/height/depth are ignored. Omit for a free-size appliance. types oven and dishwasher have exactly one model each and need no value here.") })).min(1).describe("Elements to create. At least 1. Whole batch is ONE undo step."),
    },
  },
  {
    name: "edit_elements",
    title: "Edit element properties (batch)",
    description: "THE universal editor: change ANY user-editable properties of one or MANY elements in ONE transactional call. Each op: exact name \u002B any of x/y/z (METERS), width/height/depth (MM), rot_x/rot_y/rot_z (DEGREES), locked, transparent, material, gap_left/right/top/bottom/front/back (MM), opening mode, assembled-facade fill, is_open, corner_radius, drawer params (drawer_system/drawer_type/drawer_length/drawer_color/internal_width/is_double/is_upper/paired_drawer_name/attached_facade_name), table leg_inset_mm/tabletop_material/legs_material, pillar mid_height_mm/diameter_mm, screw leg screw_thread/screw_thread_length_mm/screw_insertion_mm/screw_base_diameter_mm/screw_base_height_mm, window tint/sill_protrusion_mm, door sash_type, attached_to_name (board only: the board or facade this board is bolted to \u2014 it then follows its parent on move, rotate and open; resize is never propagated). Type-specific fields are validated against the element\u0027s actual type. Atomic: if ANY op is invalid, NOTHING is applied. Geometry changes are ONE undo step. dry_run:true simulates (applies, reports per-op state \u002B violations, reverts) \u2014 use it BEFORE risky moves/resizes.",
    kind: "write",
    inputSchema: {
      ops: z.array(z.object({ name: z.string().min(1).describe("Exact element name."), new_name: z.string().optional().describe("Rename the element to this new name. Must be unique among all elements (case-insensitive) and match ^[A-Za-z0-9_-]\u002B$ \u2014 latin letters, digits, \u0027-\u0027 and \u0027_\u0027 only; no spaces, no cyrillic. Omit to keep."), x: z.number().finite().optional().describe("Target X in METERS. Omit to keep."), y: z.number().finite().optional().describe("Target Y in METERS. Omit to keep."), z: z.number().finite().optional().describe("Target Z in METERS. Omit to keep."), width: z.number().int().min(1).optional().describe("New width (X) in MM. Omit to keep. Rejected for drawers (their size is parametric)."), height: z.number().int().min(1).optional().describe("New height (Y) in MM. Omit to keep. Rejected for drawers."), depth: z.number().int().min(1).optional().describe("New depth/thickness (Z) in MM. Omit to keep. Rejected for drawers."), rot_x: z.number().finite().optional().describe("Rotation around X in DEGREES. Omit to keep."), rot_y: z.number().finite().optional().describe("Rotation around Y in DEGREES. Omit to keep."), rot_z: z.number().finite().optional().describe("Rotation around Z in DEGREES. Omit to keep."), locked: z.boolean().optional().describe("Lock (true) / unlock (false). Unlock ONLY with the user\u0027s explicit permission. Omit to keep."), transparent: z.boolean().optional().describe("Draw the element see-through (true) or solid (false). Visual only \u2014 size, position and the specification are unaffected. Omit to keep."), material: z.string().optional().describe("Material decor id or display name (see list_materials). Omit to keep."), gap_left: z.number().int().min(0).optional().describe("Gap in MM on the left side. Omit to keep."), gap_right: z.number().int().min(0).optional().describe("Gap in MM on the right side. Omit to keep."), gap_top: z.number().int().min(0).optional().describe("Gap in MM on the top side. Omit to keep."), gap_bottom: z.number().int().min(0).optional().describe("Gap in MM on the bottom side. Omit to keep."), gap_front: z.number().int().min(0).optional().describe("Gap in MM on the front side (\u002BZ). Omit to keep."), gap_back: z.number().int().min(0).optional().describe("Gap in MM on the back side (-Z). Omit to keep."), mode: z.enum(["front_left", "front_right", "front_top", "front_bottom", "back_left", "back_right", "back_top", "back_bottom", "edge_top_left", "edge_top_right", "edge_bottom_left", "edge_bottom_right", "drawer_out", "drawer_in", "drawer_right", "drawer_left", "drawer_up", "drawer_down"]).optional().describe("Facade/window/door: opening mode. Facades accept all 18 modes; windows and doors accept front_* only. Omit to keep."), fill: z.enum(["blind", "glass", "open"]).optional().describe("Assembled facade only: center fill \u2014 blind (panel), glass (vitrine), open (empty). Omit to keep."), is_open: z.boolean().optional().describe("Facade/window/door/oven/dishwasher: true = open, false = close (the oven and dishwasher doors drop DOWN around their bottom edge; the dishwasher takes its attached furniture facade with it). Omit to keep. For drawers use cycle_drawer_animation."), corner_radius: z.number().int().min(0).optional().describe("Corner rounding radius in MM. Radial shelf: clamped to 1..min(width, depth). Stool, chair, sofa (the seat body) and pouffe (the whole upholstered box): clamped to 0..min(width, depth)/2 \u2014 0 is a square seat, the maximum is a fully round one (a circle when width == depth, a capsule otherwise). Omit to keep."), seat_height: z.number().int().min(0).optional().describe("Chair, sofa and both toilets: height of the seat TOP above the floor in MM. Chair: clamped to 80..height-50 \u2014 the legs and the backrest each keep at least 50 mm. Sofa: it is the height of the solid base block, clamped to 150..height-200. Compact toilet: clamped to 350..502, and what is left up to the fixed 790 mm becomes the cistern. Wall-hung toilet: clamped to 350..600, and raising it PUSHES flush_plate_height up when the plate would land on the lid. Omit to keep."), flush_plate_height: z.number().int().min(0).optional().describe("Wall-hung toilet only: height of the BOTTOM of the flush plate above the floor in MM (the plate itself is 240x165 mm). Clamped to seat_height\u002B88..835 \u2014 at least 50 mm of tiling above the closed lid, and the plate top never leaves the 1000 mm envelope. Omit to keep."), rim_width: z.number().int().min(0).optional().describe("Bathtub only: width of the flat rim around the bowl in MM. Clamped to 5..(min(width,depth)-200)/2 \u2014 a bowl at least 200 mm across always stays inside. Raising it LOWERS the ceilings of bowl_radius and bowl_fillet. Omit to keep."), bowl_depth: z.number().int().min(0).optional().describe("Bathtub only: depth of the bowl below the rim in MM. Clamped to 50..height-30 \u2014 30 mm of shell always stays under the bowl, so the tub never stands with a hole in the floor. Lowering it LOWERS the ceiling of bowl_fillet. Omit to keep."), bowl_radius: z.number().int().min(0).optional().describe("Bathtub only: plan corner radius of the BOWL in MM, not of the outer shell \u2014 the shell radius is bowl_radius \u002B rim_width, which is what keeps the rim the same width in the corner as along the straight side. Clamped to 0..(min(width,depth)-2*rim_width)/2: 0 is a strictly rectangular tub, the maximum gives semicircular ends. Omit to keep."), bowl_fillet: z.number().int().min(0).optional().describe("Bathtub only: radius of the wall-to-floor fillet inside the bowl in MM. Clamped to 0..min(bowl_depth, (min(width,depth)-2*rim_width-60)/2) \u2014 the arc never reaches above the rim and never closes the bowl floor to a point. Omit to keep."), mixer_centres: z.number().int().min(0).optional().describe("Wall mixer only: distance between the two wall inlets in MM (150 by default, the standard wall spacing). Clamped to 60..body_length minus body_diameter, so both escutcheons stay inside the body ends. Omit to keep."), mixer_body_length: z.number().int().min(0).optional().describe("Wall mixer only: overall body length in MM across the flow handle and the thermostat head (270 by default). Clamped to max(150, body_diameter\u002B60)..600. Raising it RAISES the ceiling of mixer_centres. Omit to keep."), mixer_body_diameter: z.number().int().min(0).optional().describe("Wall mixer only: body diameter in MM, which is also its height (70 by default). Clamped to 30..160. Raising it LOWERS the ceiling of mixer_centres and RAISES the floor of mixer_body_length. Omit to keep."), mixer_escutcheon_reach: z.number().int().min(0).optional().describe("Wall mixer only: how far the round escutcheons stand off the wall in MM (34 by default). The body axis sits this far out PLUS its own radius, so the body rests on the escutcheon faces instead of cutting into the wall. Clamped to 10..200. Omit to keep."), mixer_spout_length: z.number().int().min(0).optional().describe("Wall mixer only: how far the spout reaches forward from the body axis in MM (110 by default); it also drops two fifths of that. Clamped to 40..400. This field, not the body, drives the element depth. Omit to keep."), mixer_outlet_diameter: z.number().int().min(0).optional().describe("Wall mixer only: diameter of the hose outlet under the body in MM (13 by default, G 1/2). Clamped to 8..40. Omit to keep."), shower_column_height: z.number().int().min(0).optional().describe("Shower column only: height of the column in MM from the diverter to the top of the gooseneck (1150 by default). This is NOT the element height: the hose loop hangs below the diverter and the bounding box covers it. Clamped to max(500, 7*riser_diameter)..2200. Omit to keep."), shower_riser_diameter: z.number().int().min(0).optional().describe("Shower column only: diameter of the vertical riser tube in MM (32 by default). Clamped to 16..60. It also sets the bend radius of the gooseneck (3.5x), so raising it RAISES the floor of shower_arm_reach. Omit to keep."), shower_head_diameter: z.number().int().min(0).optional().describe("Shower column only: diameter of the round rain head in MM (250 by default). Clamped to 80..600. It is the widest part of the column and therefore the element width. Omit to keep."), shower_head_thickness: z.number().int().min(0).optional().describe("Shower column only: thickness of the flat rain head in MM (30 by default). Clamped to 8..120. Omit to keep."), shower_arm_reach: z.number().int().min(0).optional().describe("Shower column only: distance from the WALL to the rain head axis in MM (380 by default). Clamped to wall_offset \u002B 40 \u002B bend radius .. 800 \u2014 a shorter reach than the bend would fold the gooseneck back into the wall. Omit to keep."), shower_wall_offset: z.number().int().min(0).optional().describe("Shower column only: distance from the wall to the riser axis in MM (60 by default), set by the two wall brackets. Clamped to 20..200. Raising it RAISES the floor of shower_arm_reach. Omit to keep."), shower_hand_diameter: z.number().int().min(0).optional().describe("Shower column only: diameter of the hand shower head in MM (110 by default). Clamped to 60..200; it also scales the handle. Omit to keep."), shower_hose_length: z.number().int().min(0).optional().describe("Shower column only: length of the flexible hose in MM (1000 by default). Clamped to 300..3000. The slack over the straight distance between the diverter and the handle hangs as a loop BELOW the diverter and grows the element downwards; a hose shorter than that distance is drawn straight and is not stretched. Omit to keep."), wall_device_plate_width: z.number().int().min(0).optional().describe("Socket and light switch only: width of ONE plate in MM (80 by default). Clamped to 40..200. The element width is this times wall_device_posts, so width/height/depth are COMPUTED here and setting them is refused. Omit to keep."), wall_device_plate_height: z.number().int().min(0).optional().describe("Socket and light switch only: plate height in MM (80 by default). Clamped to 40..200. Omit to keep."), wall_device_protrusion: z.number().int().min(0).optional().describe("Socket and light switch only: how far the device stands off the wall in MM (10 by default). Clamped to 3..60. It is the element depth, and it is split into frame, recess and body, so a larger value deepens the socket well rather than moving the plate. Omit to keep."), wall_device_posts: z.number().int().min(0).optional().describe("Socket and light switch only: COUNT of posts side by side, 1..3 (1 by default) - a double socket or a two-key switch. A plain count, not a length: it is NOT millimetres. Posts sit flush against each other, so this MULTIPLIES the element width in MM. Omit to keep."), switch_on: z.boolean().optional().describe("Light switch only: whether the switch is on (true by default). A lamp named by at least one switch is lit when at least one of those switches is on; a lamp no switch names keeps following the global light. Omit to keep."), switch_lights: z.array(z.string().min(1)).optional().describe("Light switch only: names of the light sources this switch controls, REPLACING the current list (send an empty array to unlink everything). Up to 12; unknown names and duplicates are dropped. One switch may control several lamps and one lamp may be controlled by several switches - the link is stored only here, on the switch. Omit to keep."), cutout_width: z.number().int().min(50).optional().describe("Cooktop only: cutout width in MM \u2014 the box that goes INTO the countertop (width/height/depth describe the 5 mm plate on top; height is the total). Clamped to 50..width-10. Omit to keep."), cutout_depth: z.number().int().min(50).optional().describe("Cooktop only: cutout depth in MM. Clamped to 50..depth-10. Omit to keep."), grooves: z.string().optional().describe("Plain board only: REPLACES the whole set of grooves. Comma-separated \u0022kind:side\u0022 pairs, kind = through|blind, side = top|bottom|left|right (side names the edge the groove runs along, in the part\u0027s own frame). Example: \u0022through:top, blind:left\u0022. Empty string removes all grooves. Groove size is fixed at 16*4*7 mm (offset*width*depth) by the CNC and cannot be changed. Duplicates of the same kind\u002Bside are rejected. Omit to keep."), texture_overlays: z.string().optional().describe("Wall/floor only: REPLACES the whole set of texture overlays \u2014 local decors on a patch of one face. Semicolon-separated \u0022side:materialId\u0022 items (semicolon, not comma: the area below already uses commas). side = a|b|c|d|e|f|all, where a..f are faces 0..5 of the box (index/2 = axis X|Y|Z, even = positive direction) and all means every face. Optional area \u0022@u,v\u002BWxH\u0022 in MM from the face\u0027s lower-left corner (axes: u along the face\u0027s right axis, v along its up axis); without it the overlay covers the whole face and keeps covering it after a resize. The picture is NOT scaled to the area \u2014 it keeps its physical tile size and repeats/crops, so the area is a window onto it. Example: \u0022a:oak; b:white@100,200\u002B800x600\u0022. Empty string removes all overlays. Omit to keep."), edge_banding: z.boolean().optional().describe("Sheet board only: glue edge banding on the OPEN ends of the part. Which ends are open is computed from the scene (an end touching another part, a wall or the floor gets no banding) and cannot be set by hand. false clears all four CSV edge columns. Omit to keep."), edge_thickness_mm: z.number().finite().optional().describe("Sheet board only: edge banding tape thickness in MM (0.1..5, fractional). Omit to keep."), edge_skip_validation: z.boolean().optional().describe("Sheet board only: suppress the EDG-01 error about an end that is only PARTIALLY covered by another part. Omit to keep."), drawer_system: z.enum(["gtv", "movento"]).optional().describe("Drawer only: runner system \u2014 gtv (bought metal box, one spec line) or movento (wooden box exploded into separate spec parts). Omit to keep."), drawer_type: z.enum(["A", "B", "C", "D"]).optional().describe("Drawer only: side height type \u2014 A=86, B=120, C=168, D=200 mm. Omit to keep."), drawer_length: z.number().int().optional().describe("Drawer only: nominal slide length in MM, one of 250/300/350/400/450/500/550/600. Omit to keep."), drawer_color: z.enum(["anthracite", "white", "black"]).optional().describe("Drawer only: GTV color. Omit to keep."), internal_width: z.number().int().min(100).optional().describe("Drawer only: internal box width in MM (min 100). Omit to keep."), is_double: z.boolean().optional().describe("Drawer only: mark as part of a DOUBLE drawer (two stacked boxes). Omit to keep."), is_upper: z.boolean().optional().describe("Double drawer only: this box is the UPPER one. Omit to keep."), paired_drawer_name: z.string().optional().describe("Double drawer only: exact name of the paired drawer element (link both ways). Empty string detaches. Omit to keep."), attached_facade_name: z.string().optional().describe("Drawer and dishwasher only: exact name of the facade acting as this element\u0027s front. Empty string detaches. Omit to keep."), attached_to_name: z.string().optional().describe("Static parts only (board, panel, shelf, table, pillar): exact name of the board or facade this element is ATTACHED to. An attached board follows its parent when the parent is moved, rotated or opened; resizing is never propagated. A facade cannot be attached to anything. Empty string detaches. Omit to keep."), leg_inset_mm: z.number().int().min(0).optional().describe("Table only: inward offset of legs from corners along X and Z, in MM. Omit to keep."), tabletop_material: z.string().optional().describe("Table and stool only: material id or display name for the tabletop (the stool seat) \u2014 see list_materials. Omit to keep."), legs_material: z.string().optional().describe("Table and stool only: material id or display name for the legs (see list_materials). Omit to keep."), mid_height_mm: z.number().int().min(50).max(100).optional().describe("Pillar only: middle cylinder height in MM (clamped 50..100). Omit to keep."), diameter_mm: z.number().int().min(20).max(200).optional().describe("Pillar only: outer diameter in MM (clamped 20..200) \u2014 width and depth are always equal. Omit to keep."), screw_thread: z.enum(["M6", "M8", "M10"]).optional().describe("Screw leg only: thread designation. Omit to keep."), screw_thread_length_mm: z.number().int().min(5).max(1000).optional().describe("Screw leg only: length of the threaded rod in MM (5..1000). The leg re-derives it from the floor when the leg is moved. Omit to keep."), screw_insertion_mm: z.number().int().min(1).max(1000).optional().describe("Screw leg only: how deep the thread goes into the part it is screwed to, in MM (1..thread length). Below the part thickness the thread sticks out the other side, which is legal. Omit to keep."), screw_base_diameter_mm: z.number().int().min(5).max(200).optional().describe("Screw leg only: diameter of the foot in MM (5..200) \u2014 width and depth are always equal. Omit to keep."), screw_base_height_mm: z.number().int().min(1).max(200).optional().describe("Screw leg only: height of the foot in MM (1..200). Omit to keep."), pouffe_seat_thickness: z.number().int().min(1).optional().describe("Pouffe only: thickness of the soft seat cushion in MM, clamped to 20..height/3. What is left of the height goes to the upholstered box below it \u2014 a pouffe has no legs. Omit to keep."), bed_double: z.boolean().optional().describe("Bed only: true = double (1800 wide, two pillows, six legs), false = single (900 wide, one pillow, four legs). Switching RESETS width/height/depth to the defaults of the new type, discarding a manual resize. Omit to keep."), bed_headboard: z.boolean().optional().describe("Bed only: true = with a headboard at the head end (-Z), false = without. Switching RESETS the height to the default of the new type (900 with a headboard, 600 without) and leaves width and length alone. Omit to keep."), tint: z.enum(["clear", "tinted"]).optional().describe("Window only: glass tint \u2014 clear (transparent) or tinted (slightly darkened). Omit to keep."), sill_protrusion_mm: z.number().int().min(0).max(200).optional().describe("Window only: windowsill outward protrusion in MM (0..200). Omit to keep."), sash_type: z.enum(["glass", "blind"]).optional().describe("Door only: sash type \u2014 glass (transparent) or blind (solid panel). Omit to keep.") })).min(1).describe("Operations to apply \u2014 one per element. Each op: exact name \u002B ANY editable properties (geometry, lock, material, facade gaps/mode/fill, drawer params, table/pillar/window/door params)."),
  ...
  },
  {
    ...
    description: "Change one setting \u2014 every setting get_settings reports can also be set here, and a change takes effect immediately in an open photo mode. Project on/off keys (send value): snap_enabled | grid_enabled | block_on_violation | auto_save | snap_verbose_log | camera_pan_free. Project numeric keys (send number): snap_threshold | grid_step | auto_save_interval | edge_partial_threshold | mouse_sensitivity | wasd_speed | arrow_speed. Photo mode on/off: photo_active (enter or leave photo mode) | photo_shadows | photo_soft_shadows | photo_anti_aliasing | photo_supersampling | photo_ambient_occlusion | photo_bloom | photo_vignette | photo_ceiling | photo_ssgi | photo_lamp_shadows | photo_hdr | photo_ao_full_res (ambient occlusion at full resolution instead of half \u2014 the half-res pass is what frays the shading along a wall/ceiling join). Photo mode numeric: photo_tonemap (0 none, 1 neutral, 2 ACES \u2014 ACES is more contrasted but shifts hue and saturation in the highlights, so neutral is the honest one for showing a decor colour) | photo_quality | photo_ambient | photo_floor_bounce | photo_ambient_sky | photo_ambient_equator | photo_bounce_max | photo_exposure | photo_contrast | photo_saturation | photo_bloom_strength | photo_bloom_threshold | photo_bloom_clamp | photo_vignette_strength | photo_sun_shadow_strength | photo_shadow_distance | photo_render_scale | photo_shadowmap | photo_lights_per_object | photo_ao_intensity | photo_ao_radius | photo_ao_direct | photo_ao_falloff | photo_ssgi_strength | photo_ssgi_radius | photo_ssgi_samples | photo_ssgi_resolution | photo_ssgi_blur. Scene visibility (walls, objects, outlines, light sources) is per-edit-mode and UI-only \u2014 not exposed here.",
    kind: "write",
    inputSchema: {
      name: z.enum(["snap_enabled", "snap_threshold", "grid_enabled", "grid_step", "block_on_violation", "auto_save", "auto_save_interval", "snap_verbose_log", "camera_pan_free", "edge_partial_threshold", "mouse_sensitivity", "wasd_speed", "arrow_speed", "photo_active", "photo_quality", "photo_shadows", "photo_soft_shadows", "photo_anti_aliasing", "photo_supersampling", "photo_ambient_occlusion", "photo_bloom", "photo_vignette", "photo_ceiling", "photo_ssgi", "photo_lamp_shadows", "photo_hdr", "photo_ao_full_res", "photo_ambient", "photo_floor_bounce", "photo_ambient_sky", "photo_ambient_equator", "photo_bounce_max", "photo_exposure", "photo_contrast", "photo_saturation", "photo_tonemap", "photo_bloom_strength", "photo_bloom_threshold", "photo_bloom_clamp", "photo_vignette_strength", "photo_sun_shadow_strength", "photo_shadow_distance", "photo_render_scale", "photo_shadowmap", "photo_lights_per_object", "photo_ao_intensity", "photo_ao_radius", "photo_ao_direct", "photo_ao_falloff", "photo_ssgi_strength", "photo_ssgi_radius", "photo_ssgi_samples", "photo_ssgi_resolution", "photo_ssgi_blur"]).describe("Setting key."),
      value: z.boolean().optional().describe("New on/off value. Send this for the on/off settings (snap_enabled, grid_enabled, block_on_violation, auto_save, snap_verbose_log, camera_pan_free, and every photo_* key that names a toggle: photo_shadows, photo_soft_shadows, photo_anti_aliasing, photo_supersampling, photo_ambient_occlusion, photo_bloom, photo_vignette, photo_ceiling, photo_ssgi, photo_lamp_shadows, photo_hdr)."),
      number: z.number().finite().optional().describe("New numeric value: snap_threshold and grid_step in MM, auto_save_interval in seconds, edge_partial_threshold in %, mouse_sensitivity / wasd_speed / arrow_speed as a multiplier. Photo mode: photo_quality 0=low 1=medium 2=high 3=custom; photo_ssgi_radius / photo_ao_radius in MM; photo_shadow_distance and photo_ao_falloff in metres; photo_shadowmap in pixels; photo_ssgi_samples and photo_lights_per_object are counts; photo_ssgi_blur in pixels (0 = no denoise); photo_bloom_clamp caps how bright one pixel may contribute to the glow, in % of white \u2014 it is what stops an open sky from smearing over the whole frame; everything else in %. Send this instead of value for those keys."),
    },
  },
  {
    name: "set_photo_camera",
    title: "Aim the photo camera",
    description: "Place the photo-mode camera. Only takes effect in photo mode (set_setting photo_active first); the pose is remembered separately from the normal-mode camera and is saved with the project. Send only the fields you want to change. Pair it with take_screenshot to judge a lighting change from several viewpoints instead of one.",
    kind: "write",
    inputSchema: {
      target_x: z.number().finite().optional().describe("Point the camera looks at, X in METERS. Omit to keep."),
      target_y: z.number().finite().optional().describe("Point the camera looks at, Y in METERS \u2014 eye height above the floor. Omit to keep."),
      target_z: z.number().finite().optional().describe("Point the camera looks at, Z in METERS. Omit to keep."),
      angle_x: z.number().finite().optional().describe("Pitch in DEGREES: 0 is level, positive looks DOWN, negative looks UP. Omit to keep."),
      angle_y: z.number().finite().optional().describe("Yaw in DEGREES around the vertical axis. Omit to keep."),
      distance: z.number().finite().min(0).optional().describe("Distance from the target in METERS. Clamped to the camera\u0027s own range. Omit to keep."),
    },
  },
  {
  ...
  },
  {
    ...
    title: "Screenshot",
    description: "Render the scene camera and save it as a PNG; returns the file path and how long the frame took. The picture is the camera\u0027s own view \u2014 no panels, no sidebar, no selection outlines \u2014 so it shows what a photo of the room actually looks like. In photo mode it carries the photo lighting and post-processing.",
    kind: "read",
    inputSchema: {
      renders: z.number().int().min(1).optional().describe("How many times to render the frame before reading it back. 1 (the default) just takes the picture. A higher count turns the call into a cost measurement: the read-back is a fixed ~15 ms tax that hides the frame cost, so time a 1-render call and an 8-render call and divide the difference by 7 to get milliseconds per frame."),
    },
  },
  {
    name: "find_objects",

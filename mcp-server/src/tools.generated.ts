...
export const GEN_TOOLS: GenTool[] = [
  ...
  },
  {
    ...
    description: "Create one or MANY elements in ONE call. Each item: unique name, type (board | wall | floor | facade | assembled_facade | radial_shelf | drawer | table | radius_table | pillar | window | door, default board), x/y/z in METERS, width/height/depth in MILLIMETERS, plus type-specific fields (gaps, fill, drawer params, ...). Atomic: if ANY item is invalid, NOTHING is created. Whole batch is ONE undo step.",
    kind: "write",
    inputSchema: {
      items: z.array(z.object({ name: z.string().min(1).describe("Unique name for the new element."), type: z.enum(["board", "wall", "floor", "facade", "assembled_facade", "radial_shelf", "drawer", "table", "radius_table", "pillar", "window", "door"]).optional().describe("Element type. Default board. wall = board acting as a structural anchor; floor ignores size/position."), x: z.number().finite().optional().describe("Position X in METERS."), y: z.number().finite().optional().describe("Position Y in METERS."), z: z.number().finite().optional().describe("Position Z in METERS."), width: z.number().int().min(1).optional().describe("Size along X in MM. Defaults: board 800, assembled facade 450, radial shelf 600, table 2000, window 900, door 900."), height: z.number().int().min(1).optional().describe("Size along Y in MM. Defaults: board 400, assembled facade 700, table 750, window 1200, door 2000."), depth: z.number().int().min(1).optional().describe("Thickness along Z in MM. Defaults: board 18, radial shelf 400 (its depth), table 1000, window/door 100.") })).min(1).describe("Elements to create. At least 1. Whole batch is ONE undo step."),
    },
  },
  {
    name: "edit_elements",

...
Отсюда важное для методики: «лицензирование целиком» надо мерить как расстояние от первой строки
лога до `Initialize engine version`, а не по полю `total connection time`. Первое — то, что
редактор реально ждёт; второе — часть, которая красиво падает, ничего не меняя.

...
| 4 | 🔶 **Частично.** Запись в `transform` убрана из `Collect` и `Diagnose`: заведены `GetFacesAt`/`GetVerticesAt`/`ValidationPositionAt` с переопределениями во всех шести классах со своей геометрией. Осталось: перенести сам `Collect`/`TryPickCandidate` в ядро | 16–24 | Снэп сцену не мутирует; `Diagnose` 82→26s, `ResizeMath` 22.6→3.7s |

...
`tools/unity.ps1`. Каждый вызов — свой холодный `Unity.exe -batchMode`.

| Что | Было | Стало |
|---|---|---|
| Один тестовый класс | ~2 мин | **~22 с** |
| Полный EditMode (2237 тестов) | 6–7 мин | **~65 с** (без сборки плеера) |
| Полный PlayMode (75 тестов) | ~3,5 мин | **~100 с** |
| Сборка WinDebug | ~3 мин | ~3 мин |
| `-RunTests -RunPlayMode` | ~20 мин | **~3 мин** (плеер больше не собирается) |

**Долгоживущий фоновый редактор в шлюзе БЫЛ и УБРАН.** Он снимал стартовые
60–90 с, но постоянно отваливался, и держать его в рабочем состоянии выходило
дороже сэкономленного. Заодно снялись и его собственные расхождения: два полных
прогона подряд в одной сессии разъезжались на `CameraControllerTests` (шаг
SmoothDamp считается от `Time.deltaTime`) и `UiAsciiSymbolsTests` (атлас шрифта
переживал прогон). Новый процесс на каждый прогон таких вопросов не задаёт.

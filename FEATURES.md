...
| Specification | SpecificationManager (IQuantifies спрашивается у КОМПОНЕНТОВ, менеджер только складывает — никаких проверок типа), ISpecificationParts (композитные детали, AssembledFacadeElement), IQuantifies (труба, фитинг, ящик GTV/Movento — каждый объявляет свои строки сам), SpecItem, SpecLine.hasDims (есть ли у строки настоящие габариты WxHxD — окно печатает их только когда true), SpecUnit, SpecTotals, SpecSections (имена разделов), PipeSpecItems (Pure/Plumbing — труба м.п. по ДН, фитинг шт по виду+ДН), GtvDrawerBoardParts (Pure/Elements — дно/задняя стенка ящика GTV, единственные реально листовые детали метабокса), EdgeBandingSpecItems (Pure/Infrastructure — кромка строкой в погонных метрах по толщине, вместо только колонок Кромка L1..W2) | SpecificationManagerTests, SpecificationExportTests, SpecificationFrozenSceneTests, SpecUnitTests, SpecItemTests, SpecTotalsTests, PipeSpecItemsTests, GtvDrawerBoardPartsTests, EdgeBandingSpecItemsTests | Infrastructure/, Pure/Infrastructure/, Pure/Plumbing/, Pure/Elements/, Elements/PipeElement.cs, Elements/PipeFittingElement.cs, Elements/DrawerElement.cs, UI/SpecificationPanelUI.cs |

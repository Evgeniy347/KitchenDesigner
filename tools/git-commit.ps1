...
.DESCRIPTION
    See AGENTS.md -> "CRITICAL: Commit timestamps". In short:
      * weekday (Mon-Fri) commits live in 19:00:00-23:59:59; weekends are unrestricted
...
if ($headEpoch) {
    ...
    $previous = [DateTimeOffset]::FromUnixTimeSeconds([int64] $baseEpoch).ToOffset([TimeSpan]::FromHours(5)).DateTime
    $stamp = Get-NextCommitTime $previous
} else {
    # First commit in the repository - anchor on the current local time.
    $stamp = Set-NonRoundSeconds (Get-AllowedTime (Get-Date))
}

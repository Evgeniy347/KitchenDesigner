...
param(
    ...
    #
    # ВКЛЮЧЕНО (20 с) — догадка, что после «Cleanup mono» Library уже
    # согласована, проверена. Три опыта «убить Unity сразу после этой строки
    # → сделать следующий холодный прогон» дали Imports: total=0, actual=0,
    # CompileScripts 0,87–0,92 с и обычные 15,9–16,5 с. Переимпорта нет,
    # грязной Library снятие не оставляет.
    #
    # 20 с — с запасом: 23 выхода из 24 занимают 0,6–1,7 с. Зависший ПОСЛЕ
    # работы процесс (замерено: 2675 с простоя) теперь стоит 20 секунд, а не
    # тайм-аута; отчёт к этому моменту уже на диске и прогон засчитан.
    # Ноль возвращает старое поведение — ждать честного выхода сколько
    # потребуется.
    [int]$DoneGraceSeconds = 20
)
...
function Invoke-Unity {
    ...
    try {
        ...
        $saidLicense = $false
        <#
            Шаг опроса — четверть секунды, и это не косметика.

            Опрос раз в 2 секунды означал, что после ВЫХОДА Unity скрипт спал
            ещё до двух секунд впустую. В замерах это выглядело как загадочная
            бимодальность: одна и та же работа стабильно печаталась то 16,3 с,
            то 18,4 с, причём время загрузки проекта в логе Unity совпадало до
            десятых. Целую серию замеров пришлось признать недействительной,
            прежде чем виноватым оказался собственный `Start-Sleep`, а не
            редактор. Шаг 250 мс: -0…2 с на прогон, в среднем около секунды.

            Дороже это не стоит: за такт читается хвост лога в 32 КБ.
        #>

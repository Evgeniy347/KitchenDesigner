...
**Список типов внутри КОМАНДЫ — та же слепота, только без сторожа вовсе.** `DeleteCommand.Execute`
снимала врезку у `WindowElement` и `DoorElement` поимённо и ничего не знала про мойку с варочной;
удаление не уничтожает объект, а гасит его через `SetActive(false)`, `OnDestroy` не зовётся — и
дыра в столешнице переживает хозяина, а после Ctrl+Z хозяин возвращается уже без дыры. Тот же
список был переписан ещё в трёх местах, и в каждом — своей длины. Лечится не третьим `is`, а
...
**Сторож, спрашивающий «объявлен ли интерфейс», не спрашивает ничего.** `ElementSpecCoverage`
считала тип покрытым по `GetComponents<IQuantifies>().Length > 0` — поэтому `GetSpecItems` из
одного `yield break` удовлетворял всех трёх сторожей ведомости, а элемент исчезал из неё молча.
Спрашивать надо у ПРОДУКТА функции: прогнать настоящий `Build` и потребовать хотя бы одну строку.
Если продукт выбирает ПЕРВЫЙ подходящий из упорядоченного списка путей, сторож-дизъюнкция
(`A || B || C`) прячет мёртвый путь — порядок выносится в одну функцию, которой пользуются и код,
и тест, и требуется исключительность. Так и нашёлся `AssembledFacadeElement` с двумя маршрутами,
второй из которых был мёртв.

**Проверка «значение входит в закрытый список» обязана быть двусторонней.** Иначе опечатка внутри
самого списка ненаблюдаема, а неиспользуемые константы читаются следующим агентом как
поддерживаемые. Каждая константа обязана быть либо выдана хотя бы одним типом, либо помечена
зарезервированной **с причиной**, и сама пометка становится ложью в тот день, когда её начинают
выдавать, — это тоже красное.

**Дорогой перебор всех типов делается ОДИН раз** и на каждом типе собирает всё, что спрашивают
все сторожа. Три отдельных прохода по 37 типам стоили набору EditMode 33 секунды сверх бюджета;
один общий ленивый проход даёт ту же чувствительность за треть цены.
...
**Функция на покадровом пути начинается с вопроса «отличается ли мой ВХОД от прошлого
кадра».** Кадр перетаскивания на сцене в 401 деталь гонял полную валидацию сцены — только
чтобы выбрать цвет подсветки, — и делал это даже когда мышь стояла. И ловушка, из-за которой
кэш не спасал: **запись в `transform` тем же значением не no-op**. Она поднимает
`hasChanged`, `SceneChangeTracker.Poll` бампит `SceneRevision`, и любой кэш, ключом которого
ревизия является, обесценивает сам себя каждый кадр.

**У сторожа, который читает ИСХОДНИК, обязана быть пара, читающая ПОСТРОЕННЫЙ продукт.**
`HintTextGuardTests` сверял два множества ключей в тексте и зеленел, пока пятнадцать значков
не выросли ни на одной панели. Совпадение двух списков в исходнике — не работающая функция;
второй сторож обязан потребовать изделие: значок, строку, ведомость.

**Фикстура, переведённая на общую панель, сбрасывает в `[SetUp]` и ФОКУС.**
`RefreshUnfocused` молча пропускает сфокусированное поле, а `EventSystem` в EditMode один на
весь прогон. Эта утечка порядка выглядит не как падение, а как устаревшее значение в
ассерте — то есть как загадка.

**Габаритная коробка не отличает «вышло наружу» от «провернулось сквозь стенку».** Открытый
люк мерили объединением AABB, и оно росло в обоих случаях одинаково. Спрашивать надо каждый
угол каждой детали в местных миллиметрах — и красное тогда называет деталь и глубину захода.

**Тест, который ставит свою камеру, первым шагом утверждает, что `Camera.main` — именно
она.** `Camera.main` это ПЕРВАЯ ВКЛЮЧЁННАЯ камера с тегом MainCamera, и в общем прогоне ею
почти никогда не оказывается твоя: соседний тест оставил свою. Два теста дублирования
зеленели в одиночку и краснели в полном прогоне, причём обоими «неверными» числами код
отвечал ПРАВИЛЬНО — для того ракурса, который видел. И `Assume.That(Camera.main, Is.Null)`
не годится вместо этого: в полном прогоне он молча уводит тест в `Inconclusive`, то есть
проверка исчезает.

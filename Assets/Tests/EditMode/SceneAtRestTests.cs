...
/// позу назад и просит владельца флага (`SceneChangeTracker.Unnoticed`) этого не заметить:
/// читатель `transform.hasChanged` в проекте обязан остаться ОДИН, иначе съеденный сигнал
/// гасит всё, что на нём висит (`InfrastructureContractTests`).
///

...
/// поэтому у детей коллайдеров быть не должно: лишний дочерний коллайдер
/// перехватывает рейкаст, и выделяется деталь вместо прибора. Примитивы
/// приходят из CreatePrimitive уже с коллайдером, поэтому каждый
/// EnsureChildren обязан снимать его сразу после создания ребёнка.
...
public class ApplianceChildGeometryTests
{
    ...
    private static void AssertOnlyTheRootIsClickable(KitchenElement el)
    {
        ...
        Assert.AreEqual(0, childColliders,
            el.DisplayTypeName + ": у детей остались коллайдеры — рейкаст попадёт в деталь, "
            + "и выделится она, а не прибор; EnsureChildren обязан снимать коллайдер "
            + "сразу после CreatePrimitive");
    }

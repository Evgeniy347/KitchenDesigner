...
public class ElementConverterTests
{
    ...
    private static readonly HashSet<string> AllCovered = new HashSet<string>
    {
        ...
        // Выводится ли связь из геометрии вместо ручного выбора, и участвует ли
        // деталь в правилах зазора (GAP-01/02): обе — чистые функции от типа,
        // как и пара выше. Ни один тип, в который конвертируют, их не
        // переопределяет (винтовая опора в TargetType не входит), так что
        // переносить между классами нечего: новый класс отвечает сам.
        "AttachIsDerived", "ParticipatesInGapChecks",
        ...
        "DisplayTypeName",
        // Поверхность под декор — какой рендерер несёт рисунок и каков её
        // физический размер: чистая функция от типа и габаритов (см.
        // TableDecorReproTests), после конвертации её даёт новый класс.
        "DecorRenderer", "DecorSurfaceMM",
        // Какой элемент показать в окне свойств вместо кликнутого (верхний ящик
        // пары показывает нижний): производная от связи, а не хранимое поле.
        "InspectedElement",
        // Некромкованные торцы: маску считает EdgeSubstrate по всей сцене на
        // каждое её изменение, в сейве её нет. После конвертации ближайший
        // проход поставит её заново — переносить нечего.
        "BareFaceMask",
        ...
        "IsFlatBoardElement",
        // Лицо элемента (ElementFront): ОБЪЯВЛЕНИЕ типа о самом себе — «лицо собрано
        // из этих именованных детей» либо «отдельной лицевой детали нет, вот почему».
        // Оно отвечает на вопрос о КЛАССЕ, а не о конкретной доске: два фасада с
        // разными размерами, зазорами и декором дают побайтово один и тот же ответ,
        // сеттера у свойства нет и в сейве его нет тоже. Переносить при конвертации
        // нечего — у нового типа своё объявление, и оно верно для него, а не для
        // прежнего. Ровно как Disposal/CutoutRole/IsFlatBoardElement выше.
        "Front",
        ...
        // хранит и переносить между типами нечего: у следующего типа свой Body.
        "Body",
    ...
    public void Reflection_AllPublicProperties_AreCovered()
    {
        ...
        // а не пропуск проверки: список типов строится из сборки, и укоротиться он может
        // только если кто-то незаметно сузил охват стража.

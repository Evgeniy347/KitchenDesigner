...
/// <see cref="SceneWithoutAnchors_APartPlacedLower_MovesTheGroundOutFromUnderTheOthers"/>:
/// без якоря землёй объявляется САМЫЙ НИЖНИЙ уровень сцены, и мувер, оказавшийся
/// ниже всех, переносит землю на себя. От порядка списка это больше не зависит
/// (свип по всем перестановкам — <c>ConnectivityWithoutAnchorTests</c>), но
/// зависит от ВСЕЙ сцены, а не от пары и не от графа: единственное правило,
/// которое нельзя досчитать по одному муверу.</summary>
public class ValidationLocalityTests
{
    ...
    /// графа: когда якоря в сцене нет вовсе, землёй считается САМЫЙ НИЖНИЙ
    /// уровень сцены. Мувер, опущенный ниже всех, переносит землю на себя — и
    /// стопка в другом конце сцены, которая землёй была, повисает. Причина
    /// теперь геометрическая, а не порядковая: тот же список в любом порядке
    /// даёт тот же ответ (<c>ConnectivityWithoutAnchorTests</c>), но ответ
    /// читается из ВСЕЙ сцены, и потому мувер меняет статус детали, которой он
    /// не касался и от которой недостижим.
    ///
    ...
    /// обязан требовать наличия якоря, либо пересчитывать землю отдельно.</summary>
    [Test]
    public void SceneWithoutAnchors_APartPlacedLower_MovesTheGroundOutFromUnderTheOthers()
    {
        ...
        Assert.IsTrue(frozen["A0"].Differs(live["A0"]),
            "Мувер лёг ниже всех и перенёс землю на себя — стопка A, стоявшая на прежнем " +
            $"нижнем уровне, обязана повиснуть: было {frozen["A0"]}, стало {live["A0"]}");
        Assert.IsFalse(frozen["B0"].Differs(live["B0"]),
            "B0 стоит НА мувере и был землёй до него (уровень тот же, что у A0), поэтому " +
            "обязан остаться чистым в обеих сценах. Красное здесь значит, что землю снова " +
            "получает ОДНА компонента — та, что раньше в списке, — а вторая на том же " +
            $"уровне краснеет ни за что: было {frozen["B0"]}, стало {live["B0"]}");
    }
}

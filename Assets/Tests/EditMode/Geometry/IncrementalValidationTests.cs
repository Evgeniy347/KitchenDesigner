...
public class IncrementalValidationTests
{
    ...
    /// ветке <c>!HasAnchor</c> объявляет землёй самый нижний уровень ВСЕЙ сцены
    /// — ответ от порядка списка больше не зависит, но зависит от всей сцены
    /// целиком: деталь, опущенная ниже всех, переносит землю на себя
    /// (закреплено в <c>ConnectivityWithoutAnchorTests</c>).
    ...
    /// выбирает землю, — иначе два ответа разъехались бы.</summary>
    [Test]
    public void SceneWithoutAnAnchor_RefusesToFreeze()
    {
        Assert.IsNull(FrozenValidation.Freeze(Anchorless(), new[] { 0 }),
            "без якоря земля берётся из всей сцены — заморозка обязана отказать");
    }

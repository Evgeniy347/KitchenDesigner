...
namespace KitchenDesigner.Tests.Geometry
{
    ...
    public class PrivateReflectionRatchetTests
    {
        private static readonly (string file, int reachIns, int named, string why)[] Budgets =
        {
            ...
            ("EditMode/HighlightConsumerRuleTests.cs", 1, 0,
                "ОБЗОР, а не обращение по имени: перечисляет объявленные поля потребителей "
                + "накладки, чтобы ни один не завёл своей сценовой геометрии. NonPublic "
                + "здесь обязателен по сути — прятать свою сценовую геометрию будут именно "
                + "в приватном статическом поле, и без флага сторож слеп к дефекту, ради "
                + "которого написан. Ни одного имени не заморожено: имена полей приезжают "
                + "из обхода и попадают только в текст сообщения. Долгом не является, "
                + "потолок стоит, чтобы обзор не оброс обращениями по имени"),
            ...
            ("EditMode/UnityMagicMethodSignatureTests.cs", 1, 0,
                "ОБЗОР, а не обращение по имени: перечисляет методы всех сценовых "
                + "компонентов, чтобы поймать магическое имя жизненного цикла с "
                + "параметрами. NonPublic здесь обязателен по сути — движок зовёт Start и "
                + "Awake приватными, и без этого флага сторож слеп ровно к тому дефекту, "
                + "ради которого написан. "
                + "Ни одного нашего имени не заморожено: имена в таблице принадлежат "
                + "движку. Долгом не является, потолок стоит, чтобы обзор не оброс "
                + "обращениями по имени"),
        };

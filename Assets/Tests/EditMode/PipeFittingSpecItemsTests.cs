...
public class PipeFittingSpecItemsTests
{
    ...
    /// <summary>Противоположный вход к прочерку заглушки: тройник без единого подключения
    /// показывает три прочерка — по числу портов, а не один и не ноль. До правки оба фитинга
    /// молчали одинаково (пустая строка после «ДН»), не разбирая, сколько портов на самом
    /// деле осталось неизвестными.</summary>
    [Test]
    public void TeeWithNothingConnected_ShowsThreeDashes_NotOneLikeTheCap()
    {
        var tee = Fitting(ElementFactory.CreatePipeTee("Tee", Vector3.zero));

        Assert.AreEqual("Тройник ДН—×—×—", NameOf(tee),
            "тройник без подключений обязан назвать три прочерка — по числу портов; заглушка "
            + "с одним портом называет один, и оба случая не должны схлопываться в одинаковую "
            + "пустую строку");
    }

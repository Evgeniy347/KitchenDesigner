...
public class CutoutOwnerGuardTests
{
    ...
    /// фабрикой из трёх — 74 из 111 на один этот тест. После каждого типа стенд
    /// ОБЯЗАН вернуться к первозданному виду — и это проверяется, а не
    /// предполагается: ноль открытых дыр и простая коробка столешницы. Разошлось —
    /// стенд ставится заново, а расхождение попадает в отчёт с именем типа, который
    /// его оставил. Так повторное использование стенда не может тихо подменить
    /// чувствительность: либо стенд чист, либо тест об этом говорит.
    ///
    /// Гость уходит со стенда через <c>SceneMembership.Leave</c> — ту самую
    /// единственную точку перехода, ради которой набор и написан, — и только потом
    /// уничтожается. Первая редакция общего стенда полагалась на
    /// <c>DestroyImmediate</c> («<c>OnDestroy</c> и есть боевой путь снятия
    /// врезки»), и собственный сторож стенда это опроверг на четырёх типах сразу:
    /// у мойки и варочной <c>OnDestroy</c> снимает врезку с учёта, но НЕ
    /// перестраивает меш хозяина (столешница оставалась на 128 вершинах вместо 24),
    /// а у окна и двери врез живёт на стороне ХОЗЯИНА
    /// (<c>Wall.AttachedWindows</c>/<c>AttachedDoors</c>) и <c>OnDestroy</c> гостя
    /// про него не знает вовсе — проём оставался в стене. Боевой путь один, и он
    /// назван в сводке класса: <c>SceneMembership.Leave</c>/<c>Return</c>, через
    /// который ходят <c>DeleteCommand</c>, <c>CreateCommand</c> и отмена
    /// размещения. Оснастка обязана ходить тем же путём, иначе её собственная
    /// уборка выглядит дефектом продукта.
    ///
    ...
    public void EveryTypeThatCutsItsHost_TakesTheHoleWithItWhenDeleted_AndBringsItBackOnUndo()
    {
        ...
        foreach (var (type, _) in EveryElementType.Makers)
        {
            ...
            // Гость уходит БОЕВЫМ путём — через SceneMembership.Leave, ту самую
            // единственную точку перехода, о которой весь этот набор. Не через
            // DestroyImmediate: OnDestroy боевым путём не является и врезку снимает
            // не до конца (см. сводку выше). Стенд после Leave обязан быть
            // первозданным, и это проверяется ниже.
            SceneMembership.Leave(guest, el);
            CommandStack.Clear();
            ...
            if (dirty != null)
            {
                offenders.Add($"{type.Name}: гость ушёл через SceneMembership.Leave, а стенд "
                    + $"остался не первозданным — {dirty}. Это тот же дефект, что и «дыра "
                    + "пережила хозяина», только пойманный на самой точке перехода, а не "
                    + "через DeleteCommand поверх неё");
                EveryElementType.ClearScene();
                CommandStack.Clear();

...
public class ElementPoolResetTests
{
    ...
    private static readonly Dictionary<string, string> Derived = new Dictionary<string, string>
    {
        ...
        ["Front"] = "объявление типа о самом себе — «лицо собрано из этих именованных "
            + "детей» либо «отдельной лицевой детали нет, вот почему». Состояния "
            + "экземпляра в нём нет вовсе: сеттера у свойства не существует, в PartData "
            + "оно не лежит, в сейв не пишется, и две доски с разными размерами, "
            + "зазорами и декором дают один и тот же ответ. Из пула такое уцелеть не "
            + "может — оно и не хранится",
    ...
    public void EveryPublicProperty_OfAPooledType_IsWritable_OrDirtiable_OrDeclaredDerived()
    {
        ...
        // теста, а не пропуск проверки: список строится из сборки и укоротиться может
        // только если охват стража незаметно сузили.

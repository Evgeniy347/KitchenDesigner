...
/// PlacementController.cs: деактивация объекта (в отличие от уничтожения)
/// OnDestroy не зовёт, поэтому снять врезку некому — и в сцене остаётся дыра от
/// объекта, которого в проекте так и не появилось.
///
/// Список типов, который тут когда-то был выписан руками, заменён вопросом
/// ICutsItsHost через SceneMembership.Leave: отмена размещения — такой же уход
/// со сцены, как удаление. Что новый тип не забудет про это, следит
/// CutoutOwnerGuardTests.</summary>

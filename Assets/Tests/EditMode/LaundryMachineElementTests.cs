...
public class LaundryMachineElementTests
{
    ...
    /// <summary>Петля лежит на СОБСТВЕННОЙ левой кромке люка, поэтому открытый люк уходит
    /// вперёд ровно на свой радиус. Утверждение сверяется с радиусом, посчитанным для того
    /// же габарита, а не с записанным числом: диаметр люка — доля ширины, и число менялось
    /// уже дважды, а свойство не менялось ни разу.
    ///
    /// Три ширины подряд отвечают и на второй вопрос: петля ПЕРЕСЧИТЫВАЕТСЯ на перестройке,
    /// а не остаётся от прежнего размера — иначе вылет на всех трёх совпал бы.</summary>
    [Test]
    public void Resizing_MovesTheHinge_SoTheHatchStillTurnsAroundItself()
    {
        var sizes = new[]
        {
            new Vector3Int(600, 1200, 600),
            new Vector3Int(900, 1800, 700),
            new Vector3Int(300, 1200, 600),
        };

        var reach = new float[sizes.Length];
        for (int i = 0; i < sizes.Length; i++)
        {
            float radius = LaundryMachineBody.HatchDiameterMM(LaundryMachineKind.Washer, sizes[i])
                * 0.5f;
            reach[i] = OpenHatchReachMM(sizes[i], i, "Hinge" + i);

            Assert.AreEqual(radius, reach[i], 0.5f,
                "открытый люк уходит вперёд от петли ровно на свой радиус: петля лежит "
                + "на его собственной левой кромке. Габарит " + sizes[i]);
        }

        Assert.Greater(reach[1], reach[0] + 1f,
            "шире корпус — больше люк (доля ширины) — дальше вылет; равные вылеты означали "
            + "бы петлю, оставшуюся от прежнего размера");
        Assert.Less(reach[2], reach[0] - 1f,
            "и обратно: уже корпус — короче вылет. Пара нужна целиком, одного конца мало");
    }

...
public class GroovePartTests
{
    ...
    public void Build_IdenticalGrooves_GroupTogether()
    {
        ...
        // строку доски по unit, а не по индексу [0] (порядок строк решает материал).
    ...
    public void ToCsv_HasGroovesColumnWithKindAndSide()
    {
        ...
        // детали открыты, и Build добавляет ещё строку кромки погонными метрами — порядок
        // строк решает материал, не факт присутствия кромки. Строку доски находим по колонке
        // Name, а не по фиксированному индексу — так замер переживёт появление новых
        // строк-спутников до/после доски.

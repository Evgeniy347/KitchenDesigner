...
public class MasonryUnitTableTests
{
    ...
    public void MasonryUnit_AeratedBlock_Is600x300x200_ButGost31360DoesNotPrescribeThatSize()
    {
        ...
        Assert.AreEqual(MasonryUnit.AeratedBlockStandard, unit.Source,
            "ссылка ведёт на действующую редакцию: ГОСТ 31360-2007 отменён, заменён "
            + "ГОСТ 31360-2024 (введён 01.01.2025, переходный период истёк 10.01.2026). "
            + "Сам размер 600×300×200 нормативным НЕ является и таблицы в стандарте не "
            + "имеет: в редакции 2007 таблица 1 пункта 4.2.2 задаёт МАКСИМАЛЬНЫЕ размеры "
            + "блока 625×500×500, в редакции 2024 — 1500×650×1000, а 600×200×300 "
            + "встречается лишь как пример условного обозначения в 4.2.5. Размер заводской, "
            + "стандарт назван за марку и класс, и менять его вместе с размером нельзя: "
            + "строка уходит пользователю в ведомость");
    }

...
public class McpUiPropertyParityTests
{
    ...
    public void EveryPropertyThePanelCanEdit_IsAlsoEditableThroughMcp()
    {
        ...
        Assert.IsEmpty(missing,
            "человек правит это свойство в панели, а агент через edit_elements — нет. "
            + "Проект, собранный агентом, будет отличаться от собранного руками, и никакой "
            + "ошибки при этом не покажут: MCP просто промолчит о свойстве. Завести "
            + "проволочное поле в EditOp и правку в ElementEditAppliers (AGENTS.md → "
            + "«New property checklist», пункт 3) либо, если это решение о продукте, — "
            + "запись с причиной в UiOnly. Только в панели: " + string.Join(", ", missing));
    }
    ...
    public void EveryPropertyMcpCanEdit_IsAlsoEditableInThePanel()
    {
        ...
        Assert.IsEmpty(extra,
            "агент правит это свойство, а человек в панели — нет. Свойство, которого нет в "
            + "панели, человек не увидит и не отменит: он не поймёт, откуда взялось "
            + "состояние, которое он не задавал. Завести строку в *FieldsEditor (AGENTS.md → "
            + "«New property checklist», пункт 2) либо запись с причиной в McpOnly. "

...
public class IsoScreenshotTests : ElementFrameTests
{
    ...
    /// ElementHighlighter подмешивает элементу красный тон нарушения. Восемнадцать
    ...
    /// имеет опоры». ElementHighlighter подмешивает нарушителю тон нарушения —
    ...
    /// <summary>Шесть фитингов трассы. Кадр здесь — проверка ЧИТАЕМОСТИ, а не
    /// формы: арифметика ног и ящика проверена под dotnet в PipeFittingSpecTests,
    /// сходимость меша с ящиком и намотка торцов — в PipeFittingMeshTests. Кадру
    /// остаётся вопрос, на который ни один из них не отвечает: узнаётся ли деталь
    /// глазами — виден ли глухой торец заглушки и в какую сторону смотрит стрелка
    /// потока у подачи и обратки. Рамка строится по объединённым границам
    ...
    /// (<c>IsoCameraRig.ViewDir</c>), а что стекло люка и панель ДЕЙСТВИТЕЛЬНО попали в
    ...
    public IEnumerator IsoLightSource_Default()
    {
        ...
        // Это не тинт: светильник ЖЁЛТЫЙ, а тинта в кадре нет вовсе — оснастка
        // кадровых тестов гасит тон нарушения (ElementFrameTests), а ElementKind.Decor
        // выводит светильник из парных проверок и из опоры. Правило под мебель до
        // светильника не дотягивается, и кадр это фиксирует, чтобы следующий замер
        // цвета не отправил чинить исправное.

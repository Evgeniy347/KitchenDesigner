...
///
/// Здесь двигается КАМЕРА, а не элемент — и теперь так же устроен общий
/// съёмщик: сторона съёмки выводится из объявленного лица
/// (IsoCameraRig.ViewDir), а не вписывается разворотом по одному типу.
/// Стойка садится задней гранью на плоскость стены z = 0 и растёт в +Z: с
/// сырого IsoDir, стоящего на -Z,
...
public class ShowerColumnViewpointTests : ElementFrameTests
{
    ...
    /// ElementHighlighter подмешивает ей тон нарушения. Все пять кадров этого

...
namespace KitchenDesigner.Core.UI
{
    public class HelpUI : MonoBehaviour
    {
        ...
        private const string BodyText = "" +
            ...
            "Escape  — один хозяин на нажатие: перетаскивание → пикер связи со светом → "
            + "измерение → пипетка → подтверждение удаления → контекстное меню → меню группы → "
            + "плитка каталога → каталог → панель дня/ночи → музыка\n" +
            ...
            "3  — вид спереди\n" +
            "F1 — эта справка\n" +
            "F9  — замер производительности: включить и показать табло кадра\n" +
            "Shift + F9  — начать / остановить запись замера в файл "
            + "test-results/perf/perf_<дата>.csv";
    }
}

...
namespace KitchenDesigner.Core
{
    ...
    public class ResizeHandleManager : MonoBehaviour
    {
        ...
        private void FinishDrag()
        {
            ...
            // Обычная деталь обязана лечь на мм-сетку: снэп ставит грань
            // заподлицо к соседу, стоящему на 0.5 мм, и дробная грань разносила
            // заразу дальше. Выравниваем ДО снятия afterPos. Труба и фитинг тут
            // не при чём — MmGrid их не трогает вовсе, а целая длина трубы идёт
            // из того, что PipeElement.LengthMM — int (PipeRunFitTests).

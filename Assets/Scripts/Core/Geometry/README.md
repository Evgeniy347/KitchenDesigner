...
пустом месте: у винтовой опоры Ø25×58 резьба Ø6 занимает верхние 50 мм, и
коробка цепляет соседей, до которых резьбе 16 мм.

Исключение из правила уже есть и оно единственное: `ValidationElement`
несёт ВТОРУЮ коробку — `RecessedBody`/`HasRecessedBody` для утопленной
варочной, и её проверяет отдельный `CheckRecessedBodyAgainstCarcass`. Любая
следующая деталь, чья форма не описывается одной коробкой, идёт этой же
дорогой: вторая коробка в снимке, а НЕ переделка `GetVertices`/`GetFaces` —
на восемь вершин и шесть граней завязаны снэп, ресайз, кромки, подсветка и
`EdgeOutlineRenderer`.

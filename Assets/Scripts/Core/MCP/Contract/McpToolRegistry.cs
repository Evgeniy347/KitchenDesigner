    ...
    public static class McpToolRegistry
    {
        public static readonly IReadOnlyList<McpToolDef> Tools = new List<McpToolDef>
        {
            ...
            new McpToolDef("create_elements", "Create elements (batch)",
                "Create one or MANY elements in ONE call. Each item: unique name, type (board | wall | floor | facade | assembled_facade | radial_shelf | panel | drawer | table | radius_table | pillar | window | door | cooktop | oven, default board), x/y/z in METERS, width/height/depth in MILLIMETERS, plus type-specific fields (gaps, fill, drawer params, ...). cooktop and oven are built-in appliances: an oven and a cooktop with model:... take their size from the manufacturer and ignore width/height/depth. panel = ДВП/ХДФ back panel whose gaps count toward its bounding box, so it seats into grooves. Atomic: if ANY item is invalid, NOTHING is created. Whole batch is ONE undo step. For architecture (walls, room floors, windows/doors) prefer the corner-anchored tools apply_floorplan / create_walls / create_floor / add_opening — they derive the geometry server-side instead of making you compute centers.",
